...
public class ViewProductController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        try {
            String proName = replaceUnderscoreWithSpace(request.getParameter("value"));
            ProductDAO dao = new ProductDAO();
            ProductDTO product = dao.getPro(proName);
            HttpSession session = (HttpSession) request.getSession();           
            
            if (product != null) {
                session.setAttribute("PRODUCT", product);
                session.setAttribute("ERROR_ADD", "");
                url = SUCCESS;
            }
        } catch (Exception e) {
    ...
    public String replaceUnderscoreWithSpace(String inputString) {
